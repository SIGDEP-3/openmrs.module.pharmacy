...
<openmrs:require privilege="Manage Pharmacy" otherwise="/login.htm" redirect="/module/pharmacy/operations/transfer/editFlux.form" />
<script>
    if (jQuery) {
        ...
        function goToSelectedProduct() {
            ...
            if (productId) {
                location.href = '${pageContext.request.contextPath}/module/pharmacy/operations/transfer/editFlux.form?' +
        ...
        <div class="col-6 text-right">
            <c:if test="${productTransfer.operationStatus != 'VALIDATED' &&
                ...
                <c:if test="${productTransfer.operationStatus == 'NOT_COMPLETED' && fct:length(productAttributeFluxes) != 0}">
                    <c:url value="/module/pharmacy/operations/transfer/complete.form" var="completeUrl">
                ...
                <c:if test="${productTransfer.operationStatus != 'NOT_COMPLETED'}">
                    <c:url value="/module/pharmacy/operations/transfer/incomplete.form" var="incompleteUrl">
                        <c:param name="transferId" value="${productTransfer.productOperationId}"/>
                    </c:url>
                    <button class="btn btn-primary btn-sm mr-2" onclick="window.location='${incompleteUrl}'">
                        <i class="fa fa-pen"></i> Editer l'inventaire
                    </button>
                    <c:url value="/module/pharmacy/operations/transfer/validate.form" var="validationUrl">
                        <c:param name="transferId" value="${productTransfer.productOperationId}"/>
                    </c:url>
                        ...
                        <c:if test="${productTransfer.operationStatus == 'NOT_COMPLETED'}">
                            ...
                            <c:forEach var="productFlux" items="${productAttributeFluxes}">
                                <tr>
                                    ...
                                    <td>
                                        <c:if test="${productTransfer.operationStatus == 'NOT_COMPLETED'}">
                                            <c:url value="/module/pharmacy/operations/transfer/editFlux.form" var="editUrl">
                                                <c:param name="transferId" value="${productTransfer.productOperationId}"/>
                                                <c:param name="fluxId" value="${productFlux.productAttributeFluxId}"/>
                                            </c:url>
                                            <a href="${editUrl}" class="text-info"><i class="fa fa-edit"></i></a>
                                            <c:url value="/module/pharmacy/operations/transfer/deleteFlux.form" var="deleteUrl">
                                                <c:param name="transferId" value="${productTransfer.productOperationId}"/>
                                                <c:param name="fluxId" value="${productFlux.productAttributeFluxId}"/>
                                            </c:url>
